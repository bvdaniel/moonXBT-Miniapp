...
export default function UpdatedAirdropComponent() {
  ...
  const allRequiredCompleted =
    isConnected && tasks.length > 0 && tasks.filter((task) => task.isRequired).length === tasks.filter((task) => task.isRequired).length;
  ...
  const refreshButton = (
    <Button
      onClick={() => setIsVerifyingAll(true)}
              ...
              {isVerifyingAll && !task.isCompleted ? (
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <Button
                    ...
                    disabled={isVerifyingAll || !user?.fid}
